...
class Trade(Broker):  # 继承Brkoer类
    ...
    def set_cash(self,trade:dict,tradetime:str):
        ...
        for i in self.stock_pool:
            ...
            if tradetime in d[i].index:
                ...
                self.cash = self.cash + trade[i]['sell']*close - trade[i]['buy']*close -(trade[i]['buy']+trade[i]['sell'])*(self.commission + self.slipper_rate)# 减去交易成本
                # print(self.cash)
            else:
    ...
    def set_asset(self,trade:dict,tradetime:str, pos:dict):
        d = np.load('stock_data.npy', allow_pickle=True).item()
        for i in self.stock_pool:
            d[i].index = d[i]['trade_date']
            if tradetime in d[i].index:
                close = d[i].loc[tradetime]['close']
                self.asset = self.asset + pos[i]*close + trade[i]['sell']*close - trade[i]['buy']*close -(trade[i]['buy']+trade[i]['sell'])*(self.commission + self.slipper_rate)# 减去交易成本
                # print(self.cash)
            else:
                self.asset = self.asset   # 停牌就不交易
                # print(self.cash)
        return self.asset

# 四、五、 输出策略回测数据以及可视化图表
class Calculation():    # 计算统计各种交易数据
    def __init__(self,asset:list,basicreturn:list):
    ...
    def weekly_return(self):            # 月收益率
        week_return = []
        for i in range(1,len(self.asset)):
            tempt = (self.asset[i] - self.asset[i-1])/self.asset[i-1]   #每一期的月收益率
    ...
    def final_annualized_return(self):   # 最终年化收益率（算数平均）
        weekly_avg_return = []
        for i in range(1,len(self.asset)):
            tempt = (self.asset[i] - self.asset[i-1])/self.asset[i-1]   #每一期的周收益率
    ...
    def Sharpe_Ratio(self):              #策略夏普比: 年化收益率/收益率标准差(年)
        ...
        std_weekly = np.std(self.weekly_return(),ddof=1)
        print("周标准差：",std_weekly)
        std_yearly = std_weekly*(52**0.5)            # 周收益率 乘以根号52
        print("年标准差：",std_yearly)
        ratio = r/std_yearly
        print("夏普比率为：",ratio)
        return ratio
    ...
    def max_drawback(self):              # 最大回撤率
        ...
        for i in  range(1,500):  # 500次交易
            new_cashlist = self.asset[0:i].copy()
    ...
    def profit_count(self):             # 盈利次数
        count = 0
        for i in range(1,len(self.asset)):
            if self.asset[i] > self.asset[i-1]:
                count += 1
    ...
    def profit_loss_ratio(self):        # 盈亏比
        ...
        for i in range(1,len(self.asset)):
            if self.asset[i] > self.asset[i-1]:
                profit += self.asset[i] - self.asset[i-1]
            else:
                loss += self.asset[i-1] - self.asset[i]
        print ("总盈利额为：",profit)
        print("总亏损额为：",loss)
        ratio = profit/loss
        print("盈亏比为：",ratio)
    ...
    def week_return_comp(self):              # 策略收益率（周）+与基准收益率对比 曲线图
        x = pd.date_range(start='2010-01-05',end='2020-01',freq='W')
        y1 = []
        for i in range(1,522):
            week_return = (self.asset[i]-self.asset[i-1])/self.asset[i-1]
            y1.append(week_return)
        plt.plot(x,y1,label='strategy return',linewidth=3)
        plt.plot(x,self.basic_return,label='basic return',linewidth=1)
        plt.legend()
        plt.show()


    def excess_return_curve(self):             # 超额收益曲线图（周）
        x = pd.date_range(start='2010-01-11',end='2020-01',freq='W')
        y1 = []
        for i in range(1,521):
            week_return = (self.asset[i]-self.asset[i-1])/self.asset[i-1]
            ex_return = week_return - self.basic_return[i]
            y1.append(ex_return)
        plt.plot(x,y1,label='excess return')
        plt.legend()
        plt.show()



    def drawback_curve(self):                  # 回撤曲线（周）
        x = pd.date_range(start='2010-01-05',end='2020-01',freq='W')
        cum_return = []
        max_cum_return = []
        drawback = []
        for i in range(1,522):
            a = self.asset[i] - self.asset[0]
            cum_return.append(a)
    ...
    第二步 周末调仓 Trade(stockpool:list, trade:dict, tradedate:str, pos:dict, cash_initial)
        ...
        当期计算得到的set_cash和set_position返回结果对应下一期的cash_initial和pos
            需要写一个for循环：
                ...
                3. 存储每一期的cash作为一个列表，作为第三步计算回测结果的输入参数
